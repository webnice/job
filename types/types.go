...
type BaseInterface interface {
	...
	// Worker Функция-реализация процесса, данная функция будет запущена в горутине до тех пор, пока функция не
	// завершился воркер считается работающим.
	Worker() error
}
...
type Configuration struct {
	...
	// - если =true - в случае остановки процесса с ошибкой, сигнал остановки отправляется всем процессам приложения.
	...
	// ВНИМАНИЕ, Важно понимать когда можно устанавливать это значение, так как CancelTimeout может привести
	// к утечке горутин.
	CancelTimeout time.Duration
}

// Pith Основная структура данных процесса.
type Pith struct {
